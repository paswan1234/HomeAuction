...
class SeoSettingsController extends Controller {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions' => array('admin', 'delete', 'add', 'uploadPropertyCSV','metaTags'),
                'roles' => array('admin'),
            ),
    ...
    public function actionAdd() {
        ...
        if (isset($_POST['SeoSettings'])) {
            foreach ($_POST['SeoSettings'] as $key => $value):
                Yii::app()->db->createCommand('update seo_settings set seo_val = "' . mysql_escape_string($value) . '" where seo_key = "' . mysql_escape_string($key) . '"')->query();
            endforeach;
        }

        $seosettings = $model->findAll();
